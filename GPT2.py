...
class Dataloader:
    ...
    def get_batch(self):  # 所有批次的上下文序列长度之和
        ...
        if end_pos > len(self.tokens):  # 结束位置超过总长度时  4097>156
            add_data = end_pos - len(self.tokens) - 1  # 计算超出量，-1以适应索引  4097-156-1  3940
            end_pos = len(self.tokens) - 1  # 重新设定结束位置  155
        
        d = self.tokens[strat_pos:]  # d = [0, b * c - 1]  0, 155 = 155  shape=(155,)
        if add_data != -1:  # 进入
            d = torch.cat([d, self.tokens[:add_data+1]])  # 拼接操作  (155, 0)+(3939, 0)=4094
        x = (d[:-1]).view(b, c)  # 输入
...
train_split = 0.8  # 用于训练的总数据百分比，也就是8成数据用于训练，剩下的用于测试
...
class MultiHead_Attention(nn.Module):
    ...
    def forward(self, inputs):
        ...
        query = self.query(d_model).view(batch_size, seq_length, self.num_heads, self.head_dim).permute(0, 2, 1, 3)
        key = self.key(d_model).view(batch_size, seq_length, self.num_heads, self.head_dim).permute(0, 2, 1, 3)
        value = self.value(d_model).view(batch_size, seq_length, self.num_heads, self.head_dim).permute(0, 2, 1, 3)
        ...
        attention_out = torch.matmul(attention_scores, value).permute(0, 2, 1, 3).contiguous().view(batch_size, seq_length, d_model)
...
for ep in range(epochs):
    ...
    if ep % eval_step == 0 or ep == epochs - 1:
        ...
        with torch.no_grad():  # 关闭梯度
            ...
            print(f'Epochs: {ep}tlr: {lr}ttrain_loss: {loss}teval_loss: {e_loss}')
        m.train()
